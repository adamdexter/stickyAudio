...
# directory is a symlink-attack vector and collides between users.
BUILD_DIR="$(mktemp -d)"
...
# Bundle the canonical toggle script (single source of truth — an embedded
# copy here once drifted and lacked the mkdir guard for first-run).
cp "$SCRIPT_DIR/stickyaudio-toggle.sh" "$BUILD_DIR/toggle.sh"
